$(function () {
        ...
        if (jsonObj.status == 1) {
          ...
          $(pageableContentObj).each(function (index, board) {
            ...
            $boardCopy
              ...
              .html(board.roundReviewBoardNo);
            $boardCopy
              .find("img.board-list__content__image")
              .attr(
                "src",
                "../roundreview_images/" +
                  board.roundReviewBoardNo +
                  "/image_1.png"
              );
            $boardCopy
              .find("div.board-list__content__title")
              .html(board.roundReviewBoardTitle);
            $boardCopy
              .find("div.board-list__content__cmt-cnt")
              .html(board.roundReviewBoardCmtCnt);
            $boardCopy
              .find("div.board-list__content__nickname")
              .html(board.userNickname);
            $boardCopy
              .find("div.board-list__content__dt")
              .html(board.roundReviewBoardDt);
            $boardCopy
              .find("div.board-list__content__view-cnt")
              .html(board.roundReviewBoardViewCnt);
            $boardCopy
              .find("div.board-list__content__like-cnt")
              .html(board.roundReviewBoardLikeCnt);
  ...
  $("div.page-group").on("click", "span:not(.disabled)", function () {
    ...
    if (word == "") {
      url =
        "http://localhost:1125/backroundreview/board/list/" +
        orderType +
        "/" +
        pageNo;
    } else {
      ...
      url =
        "http://localhost:1125/backroundreview/search/" + word + "/" + pageNo;
  ...
  $("div.board").on(
    "click",
    "img.board-list__content__image, div.board-list__content__title",
    function () {
      let $roundReviewBoardNoObj = $(this)
        .parent()
        .find("div.board-list__content__no");
      let round_review_board_no = $roundReviewBoardNoObj.html();
      console.log(round_review_board_no);
      location.href =
        "../html/roundreviewdetail.html?round_review_board_no=" +
