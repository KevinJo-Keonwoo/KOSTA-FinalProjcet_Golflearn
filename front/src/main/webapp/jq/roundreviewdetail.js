$(function () {
  ...
  $.ajax({
    ...
    success: function (jsonObj) {
      if (jsonObj.status == 1) {
        ...
        $("div.board__no").html(
          "글번호&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp" +
            roundReview.roundReviewBoardNo
        );
        $("div.board__title").html(
          "제목&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp" +
            roundReview.roundReviewBoardTitle
        );
        //likelist의 usernickname이 현재 살아잇는 nickname하고 같으면 좋아요 true
        //아니면 false(색안입히기)

        $("div.user__nickname").html(
          "작성자&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp" +
            roundReview.userNickname
        );
        $("div.board__dt").html(
          "날짜&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp" +
            roundReview.roundReviewBoardDt
        );
        $("div.board__view-cnt").html(
          "조회수&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp" +
            roundReview.roundReviewBoardViewCnt
        );

        $("img.board__images1").attr(
          "src",
          "../roundreview_images/" +
            roundReview.roundReviewBoardNo +
            "/image_1.PNG"
        );
        $("img.board__images2").attr(
          "src",
          "../roundreview_images/" +
            roundReview.roundReviewBoardNo +
            "/image_2.PNG"
        );
        $("img.board__images3").attr(
          "src",
          "../roundreview_images/" +
            roundReview.roundReviewBoardNo +
            "/image_3.PNG"
        ...
        kakao.maps.event.addListener(marker, "click", function (mouseEvent) {
          ...
          infowindow.setContent(
            '<div style="padding:5px; font-size:12px;">' + msg + "</div>"
          );
  ...
  $("div.comment-box").on("click", "button.comment-box__send", function () {
    ...
    let roundReviewCmtContent = $(this)
      .siblings("input[name=comment-box__write]")
      .val();
