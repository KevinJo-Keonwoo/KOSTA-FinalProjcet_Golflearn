$(function () {
	...
	$("div.boardlist").on(
		"click",
		"div.qnaboard>div.cell>div.summary>div.board_title",
		function () {
		if ($(this).hasClass("down")) {
			...
			let $modify = $detail.find(
			"div.modifyNremove button[name=board-modify]"
			);
			let $remove = $detail.find(
			"div.modifyNremove button[name=board-remove]"
			);
			let $modifySub = $detail.find(
			"div.modifyNremove button[name=board-modify-submit]"
			);
			let $cmtInsert = $detail.find(
			"div.insertNremove button[name=comment-insert]"
			);
			let $cmtRemove = $detail.find(
			"div.insertNremove button[name=comment-remove]"
			);
			$.ajax({
			...
			success: function (jsonObj) {
				...
				if (loginedUserType == 2) {
					...
					$qnaCmtContent
						.val(board.comment.qnaCmtContent)
						.html(board.comment.qnaCmtContent);
					$qnaCmtdt
						.val(board.comment.qnaCmtDt)
						.html(board.comment.qnaCmtDt);
					$commentUserNickname
						.val(board.comment.userNickname)
						.html(board.comment.userNickname);
					$detail.show();
					$modify.hide();
					$modifySub.hide();
					$cmtInsert.hide();
	...
	$("div.boardlist").on(
		"click",
		"div.qnaboard>div.cell>div.detail>div.modifyNremove button.modify",
		function () {
		let $userNickname = $(this)
			.parents("div.cell")
			.find("div.board_nickname")
			.html();
	...
	$("div.boardlist").on(
		"click",
		"div.qnaboard>div.cell>div.detail>div.modifyNremove button[name=board-remove]",
		function () {
		let boardNo = parseInt(
			$(this).parents("div.cell").find("div.board_no").html()
		);
		$.ajax({
	...
	$("div.boardlist").on(
		...
		let qnaCmtContent = $(this)
			.parents("div.cell")
			.find("textarea[name=input_comment]")
			.val();
	...
	$("div.boardlist").on(
		"click",
		"div.qnaboard>div.cell>div.detail>div.insertNremove button[name=comment-remove]",
		function () {
		let commentNo = parseInt(
			$(this).parents("div.cell").find("div.board_no").html()
		);
		$.ajax({
