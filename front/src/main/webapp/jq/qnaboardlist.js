...
function showList(url, data) {
    ...
    showList('http://localhost:1227/qna/board/list');
    //showList('http://192.168.0.8:9997/backboardboot/board/list'); //쌤ip로 접속
    ...
    $("div.pagegroup").on("click", "span:not(.disabled)", function () {
        ...
        let data = "";
        if (word == "") {
        url = "http://localhost:1227/qna/board/list"+pageNo;
        data = "currentPage=" + pageNo;
        } else {
